...
#define MIXER_LINE_DISABLE()  (mixCondition = true, mixEnabled = 0)
      ...
      if (mode==e_perout_mode_normal && (md->delayDown || md->delayUp)) {	// there are delay values
        if (!swOn[i].delay) {
          if (s_mixer_first_run_done) { // init delay if v <> hold with +/-10 margin
	         if (v > swOn[i].hold + 10) swOn[i].delay = md->delayUp   * (100/DELAY_STEP);
			     if (v < swOn[i].hold - 10) swOn[i].delay = md->delayDown * (100/DELAY_STEP);
          }
          swOn[i].hold = v; // store actual value as reference for next run(s)
        }
        else {
