...
#if defined(CPUM2560) // (PCBGRUVIN9X)
    //instant_vbat = (instant_vbat*1112 + instant_vbat*g_eeGeneral.txVoltageCalibration + (BandGap<<2)) / (BandGap<<3);
    //instant_vbat = (instant_vbat*(1112 + g_eeGeneral.txVoltageCalibration) + (BandGap<<2)) / (BandGap<<3);
    instant_vbat = instant_vbat * (1112 + g_eeGeneral.txVoltageCalibration) / (8*BandGap);
#else
    //instant_vbat = (instant_vbat*16 + instant_vbat*g_eeGeneral.txVoltageCalibration/8) / BandGap;
    instant_vbat = instant_vbat * (128 + g_eeGeneral.txVoltageCalibration) / (8*BandGap);
#endif
...
#endif
      /*
      if (IS_TXBATT_WARNING() && g_vbat100mV>50) {	// old code for low TX voltage warning
        AUDIO_TX_BATTERY_LOW();
      }*/

      if (g_vbat100mV>50) {			// there is a voltage value
        if (g_vbat100mV <= g_eeGeneral.vBatWarn) {
          alarm_on = true;
        }
      }
#if defined(FRSKY)
      if (TELEMETRY_STREAMING()) {	// there is a telemetry stream
        if (frskyData.rssi[1].value <= (45 + g_model.frsky.rssiAlarms[0].value)) { // alarm on RSSI TX
          alarm_on = true;
        }
        if (frskyData.analog[TELEM_ANA_A1].value <= g_model.frsky.channels[TELEM_ANA_A1].alarms_value[0]) {  // alarm on first level of A1
          alarm_on = true;
        }
        if (frskyData.analog[TELEM_ANA_A2].value <= g_model.frsky.channels[TELEM_ANA_A2].alarms_value[0]) {  // alarm on first level of A2
          alarm_on = true;
        }
      }
#endif
