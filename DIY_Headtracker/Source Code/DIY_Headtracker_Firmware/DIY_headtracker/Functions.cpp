...
void Send_i2C()
{
  ...
  chan1 = channel_value[htChannels[0]] ;
  chan2 = channel_value[htChannels[1]] ;
  chan3 = channel_value[htChannels[2]] ;
...
#if NRF24L01 == 1
int chans[3] ;
unsigned long currmillis,millis_prec ;
...
//  Serial.print("Millis : ");
//  Serial.print(millis());
//  Serial.print("  Millis_prec : ");
//  Serial.println(millis_prec); 
  radio.stopListening(); 
  chans[0] = channel_value[8] ;
  chans[1] = channel_value[7] ;
  chans[2] = channel_value[6] ;
  //radio.writeBlocking( &channel_value+6, sizeof(int) * 3,50) ;
  radio.writeFast( &chans, sizeof(int) * 3,50) ;
  radio.txStandBy() ;//Serial.println(F("failed"));
